window.WORKS = [
  {
    "slug": "commercial-blank-01",
    "title_zh": "【文旅】光影节表演",
    "title_en": "",
    "detail_title_en": "Cultural Tourism Light Festival Performance",
    "role_zh": "",
    "role_en": "Cultural Performance",
    "year": "2025",
    "category": "Commercial",
    "one_line_zh": "2025 年国庆期间的上海光影艺术节项目，静安公园主场「家园」光影装置表演。参与装置的现场搭建与落地执行，并承担表演期间的影像记录与素材拍摄，配合主办团队完成大型公共空间项目的呈现。",
    "one_line_en": "A Shanghai Light Art Festival project during the 2025 National Holiday, presenting the \"Home\" light installation performance at Jing'an Park — involved in on-site installation setup and live footage documentation.",
    "summary_zh": "2025 年国庆期间的上海光影艺术节项目，静安公园主场「家园」光影装置表演。参与装置的现场搭建与落地执行，并承担表演期间的影像记录与素材拍摄，配合主办团队完成大型公共空间项目的呈现。",
    "summary_en": "A Shanghai Light Art Festival project during the 2025 National Holiday, presenting the \"Home\" light installation performance at Jing'an Park — involved in on-site installation setup and live footage documentation.",
    "modal_full": true,
    "platform": "新片场",
    "link_label": "",
    "embed": "https://player.xinpianchang.com/?aid=13674270&mid=KdRx7bZ6Wrr7b2N3",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13674270?from=webShare&channel=copyLink&token=3BNdoKeHrbv4ugU1D4zIZqR",
    "cover": "assets/img/guang1.png",
    "cover_webp": "assets/img/guang1.webp",
    "cover_size": "95% auto",
    "credits": [
      "部分装置现场搭建与落地执行 / On-site Installation Setup & Execution",
      "现场表演影像记录与素材拍摄 / Live Performance Documentation & Footage Capture"
    ],
    "process": [
      "配合团队完成装置现场搭建 / Collaborated with team on installation setup",
      "表演期间负责影像记录与素材采集 / Handled footage documentation during performance"
    ],
    "tools": ["待补充 / TBD"],
    "stills": [],
    "sort_order": 101
  },
  {
    "slug": "bookstore",
    "sort_order": 1,
    "title_zh": "【纪实】不平凡的书店",
    "title_en": "",
    "role_zh": "",
    "role_en": "The Uncommon Bookstore",
    "year": "2023",
    "category": "Documentary",
    "one_line_zh": "以书店为切入点的人文纪录片：从选题策划、访谈设计到拍摄与后期，全流程独立完成，呈现书店作为城市文化公共空间的价值。（导演 / 摄影 / 剪辑）",
    "one_line_en": "A documentary that uses a bookstore as the entry point to examine the ties between space, people, reading, and urban memory, covering topic development, interview design, production, and post‑production to articulate the cultural value of bookstores as public spaces.",
    "platform": "bilibili",
    "embed": "https://player.bilibili.com/player.html?isOutside=true&aid=562788453&bvid=BV1Tv4y1U79v&cid=943403902&p=1",
    "link": "https://www.bilibili.com/video/BV1Tv4y1U79v",
    "cover": "assets/img/doc-bookstore-1.png",
    "cover_webp": "assets/img/doc-bookstore-1.webp",
    "credits": [
      "导演 / Director",
      "摄影 / DP",
      "剪辑 / Editor"
    ],
    "process": [
      "访谈设计 / Interview Design",
      "现场拍摄 / Shoot",
      "后期剪辑 / Edit"
    ],
    "tools": [
      "Premiere Pro"
    ],
    "stills": [
      "assets/img/bu1.png",
      "assets/img/doc-bookstore-1.png"
    ]
  },
  {
    "slug": "documentary-placeholder-01",
    "sort_order": 3,
    "title_zh": "【视频播客】失业与营销",
    "title_en": "",
    "role_zh": "",
    "role_en": "Commercial Video Podcast",
    "year": "2026",
    "category": "Documentary",
    "one_line_zh": "商业对谈类视频播客的试播集。负责双机位剪辑，并根据素材整理出可复用的后期文稿脚本——把一场松散的对谈整理成结构清晰、可直接上线的节目。",
    "one_line_en": "Two‑camera edit for a commercial video talk‑show podcast pilot, plus a post‑production script compiled from the video footage.",
    "summary_zh": "商业对谈类视频播客的试播集。负责双机位剪辑，并根据素材整理出可复用的后期文稿脚本——把一场松散的对谈整理成结构清晰、可直接上线的节目。",
    "summary_en": "Two‑camera edit for a commercial video talk‑show podcast pilot, plus a post‑production script compiled from the video footage.",
    "modal_full": true,
    "platform": "新片场",
    "embed": "https://player.xinpianchang.com/?aid=13610114&mid=GKL87NyD5xlQojJz",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13610114?from=webShare&channel=copyLink",
    "cover": "assets/img/ai1.png",
    "cover_webp": "assets/img/ai1.webp",
    "credits": [
      "剪辑 / 文稿脚本整理 / Editing / Script Organization"
    ],
    "process": [
      "整理文稿脚本 / Script Organization",
      "后期剪辑 / Two‑camera edit"
    ],
    "tools": [
      "Premiere"
    ],
    "stills": [
      "assets/img/podcast-still-1.png",
      "assets/img/podcast-still-2.png"
    ]
  },
  {
    "slug": "documentary-placeholder-02",
    "title_zh": "【访谈】具身智能行业技术",
    "title_en": "",
    "role_zh": "",
    "role_en": "Tech Founder Interview Program",
    "year": "2026",
    "category": "Documentary",
    "one_line_zh": "苏州某材料科技企业的企业家双人访谈节目。原始素材结构较散，负责重新梳理内容脉络与表达重点，统筹脚本整理、剪辑节奏与调色——让企业家的表达更专业，成片可直接用于对外传播。",
    "one_line_en": "Handled post-production for a two-person interview program featuring a Suzhou materials-tech entrepreneur, restructuring loosely organized source footage into a clear narrative while leading script consolidation, edit pacing, and color finishing to strengthen executive presence and delivery quality.",
    "summary_zh": "苏州某材料科技企业的企业家双人访谈节目。原始素材结构较散，负责重新梳理内容脉络与表达重点，统筹脚本整理、剪辑节奏与调色——让企业家的表达更专业，成片可直接用于对外传播。",
    "summary_en": "Handled post-production for a two-person interview program featuring a Suzhou materials-tech entrepreneur, restructuring loosely organized source footage into a clear narrative while leading script consolidation, edit pacing, and color finishing to strengthen executive presence and delivery quality.",
    "modal_full": true,
    "platform": "新片场",
    "embed": "https://player.xinpianchang.com/?aid=13650805&mid=NK0mw6RqdY8wW1na",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13650805?from=webShare&channel=copyLink",
    "cover": "assets/img/ju.png",
    "cover_webp": "assets/img/ju.webp",
    "credits": [
      "剪辑 / 策划 / Editing / Planning"
    ],
    "process": [
      "整理脚本框架 / Script Structuring",
      "后期剪辑 / Editing",
      "调色统一 / Color Finishing"
    ],
    "tools": [
      "Premiere"
    ],
    "stills": [
      "assets/img/su1.png",
      "assets/img/su2.png",
      "assets/img/su3.png",
      "assets/img/su4.png"
    ]
  },
  {
    "slug": "documentary-placeholder-03",
    "sort_order": 2,
    "title_zh": "【长视频播客】完美创始人",
    "title_en": "",
    "role_zh": "",
    "role_en": "Long-form Video Podcast: Perfect Founder",
    "year": "2025",
    "category": "Documentary",
    "one_line_zh": "财经访谈类视频播客节目。负责剪辑端全流程：剪辑脚本策划、粗剪精剪、字幕与包装，按更新节奏稳定交付——适配品牌做持续性内容栏目的需求。",
    "one_line_en": "Full post-production for a long-form finance interview podcast — restructured 2h 15m of multi-camera footage into a 67-minute final cut, covering script planning, editing, subtitles, and packaging.",
    "summary_zh": "财经访谈类视频播客节目。负责剪辑端全流程：剪辑脚本策划、粗剪精剪、字幕与包装，按更新节奏稳定交付——适配品牌做持续性内容栏目的需求。",
    "summary_en": "Handled full post-production for episode 2 of a long-form finance interview podcast. Starting from approximately 2h 15m of raw footage, produced a detailed timecoded edit script after full transcription and narrative structuring, then proceeded through multi-camera rough and fine cut. The final 67-minute cut was shaped around a narrative arc that preserved emotional peaks and core information density, with subtitle production and packaging completed for delivery.",
    "modal_full": true,
    "platform": "新片场",
    "embed": "https://player.xinpianchang.com/?aid=13685338&mid=GmOEQG2My06418bn",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13685338?from=webShare&channel=copyLink",
    "cover": "assets/img/wan1.png",
    "cover_webp": "assets/img/wan1.webp",
    "credits": [
      "剪辑脚本策划 / Script Planning & Edit",
      "多机位剪辑 / Multi-camera Editing",
      "字幕制作 / Subtitles",
      "包装设计与输出 / Packaging & Delivery"
    ],
    "process": [
      "转录与逻辑梳理 / Transcription & Narrative Structuring",
      "多机位粗剪与精剪 / Multi-camera Rough & Fine Cut",
      "字幕制作与时间码校对 / Subtitle Production & Timecode Review",
      "包装设计与输出交付 / Packaging & Final Delivery"
    ],
    "tools": [
      "Premiere Pro"
    ],
    "stills": [
      "assets/img/wanmei1.png",
      "assets/img/wanmei5.png",
      "assets/img/wanmei2.png",
      "assets/img/wanmei3.png"
    ]
  },
  {
    "slug": "color-reel",
    "title_zh": "商业调色作品",
    "title_en": "",
    "role_zh": "",
    "role_en": "Commercial Color Grading",
    "year": "2023",
    "category": "Commercial",
    "one_line_zh": "商业影像调色合集（持达芬奇官方调色认证）。通过对肤色、对比与情绪色彩的系统控制，统一品牌内容的画面质感，保证不同项目、不同场景下的视觉一致性。",
    "one_line_en": "A commercial color‑grading collection focused on tonal consistency and premium finishing across brand and advertising scenarios, with systematic control over skin tones, contrast, and emotional color to strengthen narrative clarity and brand identity.",
    "platform": "xinpianchang",
    "embed": "",
    "link": "https://www.xinpianchang.com/a13056017",
    "cover": "assets/img/color-commercial-2.png",
    "cover_size": "95% auto",
    "credits": [
      "调色 / Colorist"
    ],
    "process": [
      "色彩风格设定 / Look Dev",
      "统一质感 / Finishing"
    ],
    "tools": [
      "DaVinci Resolve"
    ],
    "stills": []
  },
  {
    "slug": "xinpianchang-color",
    "title_zh": "短剧调色作品",
    "title_en": "",
    "role_zh": "",
    "role_en": "Short Drama Color Grading",
    "year": "2023",
    "category": "Commercial",
    "one_line_zh": "短剧内容的调色案例。为叙事与人物关系建立统一的色彩逻辑，在多场景切换下保持整体一致——内容量大、周期紧的项目也能保住质感。",
    "one_line_en": "Short‑drama grading samples that build a stable color logic for storytelling and character relationships, enhancing light‑and‑shadow hierarchy and emotional flow while keeping consistency across scene changes.",
    "platform": "xinpianchang",
    "embed": "",
    "link": "https://www.xinpianchang.com/a13135917",
    "cover": "assets/img/color-short-2.png",
    "cover_size": "95% auto",
    "credits": [
      "调色 / Colorist"
    ],
    "process": [
      "色彩设计 / Look Dev",
      "最终调色 / Finishing"
    ],
    "tools": [
      "DaVinci Resolve"
    ],
    "stills": []
  },
  {
    "slug": "mix-2023",
    "title_zh": "【混剪】商业广告合集",
    "title_en": "",
    "role_zh": "",
    "role_en": "Commercial Ad Mix Reel",
    "year": "2023",
    "category": "Commercial",
    "sort_order": 100,
    "one_line_zh": "2023 年度商业项目混剪：把全年多个项目素材结构化重组，输出一支可直接对外展示的年度回顾片。",
    "one_line_en": "An annual commercial reel that curates highlights across projects into a structured showcase.",
    "platform": "xinpianchang",
    "embed": "https://player.xinpianchang.com/?aid=12901432&mid=3Yn57pvBWlL4dDAa",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a12901432?channel=copyLink&from=webShare",
    "cover": "assets/img/fu2.png",
    "cover_webp": "assets/img/fu2.webp",
    "credits": [
      "剪辑 / Editor"
    ],
    "process": [
      "混剪 / Reel Edit"
    ],
    "tools": [
      "Premiere Pro"
    ],
    "stills": []
  },
  {
    "slug": "you-have-to",
    "title_zh": "【电影】《You have to》",
    "title_en": "",
    "role_zh": "",
    "role_en": "Short Film Editor",
    "year": "2025",
    "category": "Short",
    "one_line_zh": "海外短片《You have to》的剪辑，负责成片的结构搭建与情绪节奏。",
    "one_line_en": "Short film editing work.",
    "platform": "vimeo",
    "embed": "",
    "link": "",
    "cover": "assets/img/have2.jpg",
    "media": "assets/img/have3.png",
    "credits": [
      "剪辑 / Editor"
    ],
    "process": [
      "剪辑 / Edit",
      "节奏控制 / Pacing"
    ],
    "tools": [
      "DaVinci Resolve"
    ],
    "stills": []
  },
  {
    "slug": "short-placeholder-02",
    "title_zh": "【电影】《Story Behind the Story》",
    "title_en": "",
    "role_zh": "",
    "role_en": "Short Film Editor",
    "year": "2025",
    "category": "Short",
    "one_line_zh": "收录于 IMDb 的短片电影《Story Behind the Story》，担任剪辑与后期制作。",
    "one_line_en": "Short film feature 'Story Behind the Story', serving as editor and post-production lead.",
    "platform": "imdb",
    "embed": "",
    "link": "https://vimeo.com/showcase/12036293?video=1149049857",
    "cover": "assets/img/have1.jpg",
    "credits": [
      "剪辑 / Editor"
    ],
    "process": [
      "剪辑 / Edit",
      "节奏控制 / Pacing"
    ],
    "tools": [
      "Premiere"
    ],
    "stills": [
      "assets/img/have4.jpg",
      "assets/img/have5.jpg"
    ]
  },
  {
    "slug": "short-placeholder-machine",
    "title_zh": "【音乐】《机器》-MV",
    "title_en": "",
    "role_zh": "",
    "role_en": "Machine MV",
    "year": "2023",
    "category": "Short",
    "one_line_zh": "独立乐队音乐影像《机器》，与团队合作完成拍摄与剪辑。",
    "one_line_en": "Collaborated with the team on the production of this music video.",
    "summary_zh": "独立乐队音乐影像《机器》，与团队合作完成拍摄与剪辑。",
    "summary_en": "Participated as part of the team in the production and post-production of this music video.",
    "platform": "xinpianchang",
    "embed": "https://player.xinpianchang.com/?aid=12414794&mid=Mk6B4xPjNZE49LAP",
    "embed_ratio": "452 / 283",
    "link": "https://music.163.com/mv/?id=14597578&userid=292743521",
    "cover": "assets/img/machine-mv-1.png",
    "cover_size": "95% auto",
    "hide_credits_process": true,
    "credits": [],
    "process": [],
    "tools": [
      "Premiere Pro"
    ],
    "stills": []
  },
  {
    "slug": "short-placeholder-tianwang",
    "title_zh": "【音乐】《天王巨星》-MV",
    "title_en": "",
    "role_zh": "",
    "role_en": "Machine MV",
    "year": "2024",
    "category": "Short",
    "one_line_zh": "独立乐队音乐影像《天王巨星》，与团队协作完成，从拍摄到成片。",
    "one_line_en": "Collaborated with the team on the production of this music video.",
    "summary_zh": "独立乐队音乐影像《天王巨星》，与团队协作完成，从拍摄到成片。",
    "summary_en": "Participated as part of the team in the production and post-production of this music video.",
    "platform": "bilibili",
    "embed": "https://player.bilibili.com/player.html?isOutside=true&aid=113016427250391&bvid=BV1jbWZekEbG&cid=500001660686570&p=1",
    "embed_ratio": "16 / 9",
    "link": "https://www.bilibili.com/video/BV1jbWZekEbG",
    "cover": "assets/img/tianwang-mv-cover.jpg",
    "cover_webp": "assets/img/tianwang-mv-cover.webp",
    "cover_size": "95% auto",
    "hide_credits_process": true,
    "credits": [],
    "process": [],
    "tools": [
      "Premiere Pro"
    ],
    "stills": []
  },
  {
    "slug": "placeholder-01",
    "title_zh": "【IP】商业穆桂英",
    "title_en": "",
    "detail_title_en": "Mu Guiying Business IP",
    "role_zh": "",
    "role_en": "Short-video IP",
    "year": "2025",
    "category": "ShortVideo",
    "one_line_zh": "2025 年运营商业 IP 视频号「穆桂英雪姐」，聚焦女性商业与心理学方向，累计产出约 100 条内容。从文案、拍摄、剪辑包装到发布排期全环节独立负责——一个人即可维持账号的稳定更新产能。",
    "one_line_en": "",
    "summary_zh": "2025 年运营商业 IP 视频号「穆桂英雪姐」，聚焦女性商业与心理学方向，累计产出约 100 条内容。从文案、拍摄、剪辑包装到发布排期全环节独立负责——一个人即可维持账号的稳定更新产能。",
    "summary_en": "",
    "modal_full": true,
    "platform": "视频号",
    "link_label": "穆桂英雪姐（视频号）",
    "embed": "",
    "link": "",
    "cover": "assets/img/lixue5-1400.jpg",
    "cover_webp": "assets/img/lixue5-1400.webp",
    "cover_size": "95% auto",
    "credits": [
      "文案 / 拍摄 / 剪辑"
    ],
    "process": [
      "拍摄 / 剪辑 / 调色结构说明 / Shoot · Edit · Color",
      "后期制作 / Post‑production",
      "文案与发布 / Copy & Publish"
    ],
    "tools": [
      "剪映"
    ],
    "stills": [
      "assets/img/lixue4-1400.jpg",
      "assets/img/lixue3-1400.jpg"
    ]
  },
  {
    "slug": "shortvideo-blank-01",
    "sort_order": 1,
    "title_zh": "【品宣】GT Show 汽车展",
    "title_en": "",
    "detail_title_en": "Tesche GT Show Brand Recap",
    "role_zh": "",
    "role_en": "Brand Promotion",
    "year": "2026",
    "category": "Commercial",
    "one_line_zh": "2026 苏州 GT Show 汽车展，参展品牌特驰的展会总结品宣片。围绕品牌调性与展会传播目标完成后期制作：内容整合、节奏与色彩统一——展会结束后，把现场热度沉淀为品牌可复用的传播资产。",
    "one_line_en": "Handled post-production for Tesche's brand recap video at Suzhou GT Show 2026, shaping edit rhythm, content structure, and color consistency around the brand tone and event communication goals to amplify atmosphere and brand recall.",
    "summary_zh": "2026 苏州 GT Show 汽车展，参展品牌特驰的展会总结品宣片。围绕品牌调性与展会传播目标完成后期制作：内容整合、节奏与色彩统一——展会结束后，把现场热度沉淀为品牌可复用的传播资产。",
    "summary_en": "Handled post-production for Tesche's brand recap video at Suzhou GT Show 2026, shaping edit rhythm, content structure, and color consistency around the brand tone and event communication goals to amplify atmosphere and brand recall.",
    "modal_full": true,
    "platform": "新片场",
    "link_label": "",
    "embed": "https://player.xinpianchang.com/?aid=13640087&mid=GKL87NyE2PaQojJz",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13640087?from=webShare&channel=copyLink",
    "media": "assets/img/qiche2.jpg",
    "media_webp": "assets/img/qiche2.webp",
    "cover": "assets/img/qiche2.jpg",
    "cover_webp": "assets/img/qiche2.webp",
    "cover_size": "auto 90%",
    "credits": [
      "后期剪辑和调色"
    ],
    "process": [
      "后期剪辑 / Post-editing",
      "调色 / Color Grading"
    ],
    "tools": [
      "pr"
    ],
    "stills": [
      "assets/img/qiche3.PNG"
    ]
  },
  {
    "slug": "shortvideo-blank-02",
    "sort_order": 2,
    "title_zh": "【营销】雪碧线下活动",
    "title_en": "",
    "detail_title_en": "Sprite Offline Activation Campaign",
    "role_zh": "",
    "role_en": "Brand Promotion",
    "year": "2026",
    "category": "Commercial",
    "one_line_zh": "雪碧（Sprite）线下营销活动的品宣视频后期。按品牌的露出规范与传播节奏完成剪辑——线下活动的二次传播素材，交付即可投放。",
    "one_line_en": "In March 2026, edited a Sprite offline campaign video to sharpen pacing, strengthen brand visibility, and improve delivery impact.",
    "summary_zh": "雪碧（Sprite）线下营销活动的品宣视频后期。按品牌的露出规范与传播节奏完成剪辑——线下活动的二次传播素材，交付即可投放。",
    "summary_en": "In March 2026, edited a Sprite offline campaign video to sharpen pacing, strengthen brand visibility, and improve delivery impact.",
    "modal_full": true,
    "platform": "新片场",
    "link_label": "",
    "embed": "https://player.xinpianchang.com/?aid=13640128&mid=yOBl4zxkYMM7aMg0",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13640128?from=webShare&channel=copyLink&token=cQ2M23IUYEg1vhKDq4v9IGQ",
    "cover": "assets/img/xuebi3.JPG",
    "cover_webp": "assets/img/xuebi3.webp",
    "cover_size": "95% auto",
    "credits": [
      "后期剪辑"
    ],
    "process": [
      "后期剪辑"
    ],
    "tools": [
      "剪映"
    ],
    "stills": [
      "assets/img/xuebi1.PNG",
      "assets/img/xuebi2.JPG"
    ]
  },
  {
    "slug": "shortvideo-blank-03",
    "sort_order": 3,
    "title_zh": "【高数据】刘亚卿商业故事",
    "title_en": "",
    "detail_title_en": "Founder IP Story with 10K+ Engagement",
    "role_zh": "",
    "role_en": "Commercial IP",
    "year": "2026",
    "category": "ShortVideo",
    "one_line_zh": "企业家刘亚卿的自传型商业 IP 视频。独立完成故事线策划、内容提炼与后期制作，成片自然流量点赞与转发均过万。",
    "one_line_en": "Produced an autobiographical commercial IP video for Chinese entrepreneur Liu Yaqing, leading story development, content distillation, and post-production to strengthen personal branding and audience traction, with organic likes and shares both exceeding 10,000.",
    "summary_zh": "企业家刘亚卿的自传型商业 IP 视频。独立完成故事线策划、内容提炼与后期制作，成片自然流量点赞与转发均过万。",
    "summary_en": "Produced an autobiographical commercial IP video for Chinese entrepreneur Liu Yaqing, leading story development, content distillation, and post-production to strengthen personal branding and audience traction, with organic likes and shares both exceeding 10,000.",
    "modal_full": true,
    "platform": "新片场",
    "link_label": "",
    "embed": "https://player.xinpianchang.com/?aid=13670921&mid=E6WlQglDyNO7gdJz",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13670921",
    "cover": "assets/img/gao4.png",
    "cover_webp": "assets/img/gao4.webp",
    "cover_size": "95% auto",
    "credits": [
      "故事脚本策划和后期剪辑"
    ],
    "process": [
      "前期策划，内容策划",
      "剪辑和调色"
    ],
    "tools": [
      "剪映"
    ],
    "stills": [
      "assets/img/gao1.PNG",
      "assets/img/gao2.JPG",
      "assets/img/gao5.png"
    ]
  },
  {
    "slug": "shortvideo-placeholder-04",
    "title_zh": "【IP】幸福凝老师",
    "title_en": "",
    "detail_title_en": "Xingfu Ning IP",
    "role_zh": "",
    "role_en": "Short-video IP",
    "year": "2025",
    "category": "ShortVideo",
    "one_line_zh": "2025 年参与运营视频号「幸福凝教官」（女性商业与心理学方向），负责文案、拍摄、剪辑包装与发布的完整内容生产环节。",
    "one_line_en": "",
    "summary_zh": "2025 年参与运营视频号「幸福凝教官」（女性商业与心理学方向），负责文案、拍摄、剪辑包装与发布的完整内容生产环节。",
    "summary_en": "",
    "modal_full": true,
    "platform": "视频号",
    "link_label": "幸福凝教官（视频号）",
    "embed": "",
    "link": "",
    "cover": "assets/img/tu14-1400.jpg",
    "cover_webp": "assets/img/tu14-1400.webp",
    "cover_size": "95% auto",
    "credits": [
      "文案 / 拍摄 / 剪辑"
    ],
    "process": [
      "拍摄 / 剪辑 / 调色结构说明 / Shoot · Edit · Color",
      "后期制作 / Post‑production",
      "文案与发布 / Copy & Publish"
    ],
    "tools": [
      "剪映"
    ],
    "stills": [
      "assets/img/tu11-1400.jpg",
      "assets/img/tu12-1400.jpg",
      "assets/img/tu13-1400.jpg"
    ]
  },
  {
    "slug": "shortvideo-placeholder-02",
    "title_zh": "【IP】椰子说国际教育",
    "title_en": "",
    "detail_title_en": "Coconut Talks: International Education",
    "role_zh": "",
    "role_en": "Short-video IP",
    "year": "2022",
    "category": "ShortVideo",
    "one_line_zh": "2022 年运营短视频账号「椰子说国际教育」（国际学校与留学方向）。负责脚本文案与后期制作，平台获客线索转交销售、推动学员转化成交——做的是直接服务业务结果的内容。",
    "one_line_en": "Operated the short-video account 'Coconut Talks: International Education' part-time in 2022, focusing on international school enrollment and overseas study. Responsible for scriptwriting, post-production, and platform-based lead generation that converted prospective students into clients.",
    "summary_zh": "2022 年运营短视频账号「椰子说国际教育」（国际学校与留学方向）。负责脚本文案与后期制作，平台获客线索转交销售、推动学员转化成交——做的是直接服务业务结果的内容。",
    "summary_en": "Operated the short-video account 'Coconut Talks: International Education' part-time in 2022, focusing on international school enrollment and overseas study. Responsible for scriptwriting, post-production, and platform-based lead generation that converted prospective students into clients.",
    "modal_full": true,
    "platform": "抖音 / Douyin",
    "link_label": "抖音搜索「椰子说国际教育」",
    "embed": "",
    "link": "",
    "cover": "assets/img/yezishuo-1.jpg",
    "cover_webp": "assets/img/yezishuo-1.webp",
    "cover_size": "95% auto",
    "credits": [
      "短视频项目制作 / Short-video Project Production"
    ],
    "process": [
      "视频剪辑和直播幕后 / Video Editing & Live-stream Backstage Support"
    ],
    "tools": [
      "剪映 / CapCut"
    ],
    "stills": [
      "assets/img/yezishuo-1.jpg"
    ]
  },
  {
    "slug": "shortvideo-empty-placeholder",
    "sort_order": 999,
    "title_zh": "【专访】十年幼教路",
    "title_en": "",
    "detail_title_en": "Ten Years in Early Childhood Education",
    "role_zh": "",
    "role_en": "Interview",
    "year": "2026",
    "category": "ShortVideo",
    "one_line_zh": "《十年幼教路》人物专访，负责访谈结构与剪辑，把十年从业经历整理成一支有说服力的人物内容。",
    "one_line_en": "An interview documenting a decade-long journey in early childhood education.",
    "summary_zh": "《十年幼教路》人物专访，负责访谈结构与剪辑，把十年从业经历整理成一支有说服力的人物内容。",
    "summary_en": "An interview documenting a decade-long journey in early childhood education.",
    "modal_full": true,
    "platform": "新片场",
    "link_label": "",
    "embed": "https://player.xinpianchang.com/?aid=13670922&mid=XzVJwDGKqdM7D0EZ",
    "embed_ratio": "1672 / 941",
    "link": "https://www.xinpianchang.com/a13670922",
    "cover": "assets/img/you1.png",
    "cover_webp": "assets/img/you1.webp",
    "cover_size": "95% auto",
    "credits": [
      "待补充"
    ],
    "process": [
      "待补充"
    ],
    "tools": [
      "待补充"
    ],
    "stills": []
  },
  {
    "slug": "shortvideo-jiazhuang-01",
    "sort_order": 1000,
    "title_zh": "【家装】设计师大改造",
    "title_en": "",
    "role_zh": "",
    "role_en": "信息上传中",
    "year": "2025",
    "category": "ShortVideo",
    "one_line_zh": "家装设计师「大改造」系列短视频，负责现场拍摄与后期剪辑包装。（详情整理中）",
    "one_line_en": "信息上传中",
    "summary_zh": "家装设计师「大改造」系列短视频，负责现场拍摄与后期剪辑包装。（详情整理中）",
    "summary_en": "信息上传中",
    "modal_full": true,
    "platform": "",
    "embed": "",
    "link": "",
    "cover": "assets/img/jia1.png",
    "cover_webp": "assets/img/jia1.webp",
    "cover_size": "95% auto",
    "credits": ["信息上传中"],
    "process": ["信息上传中"],
    "tools": ["信息上传中"],
    "stills": []
  },
  {
    "slug": "nio-et9",
    "sort_order": 99,
    "title_zh": "【TVC】蔚来ET9 音响测试",
    "title_en": "",
    "role_zh": "",
    "role_en": "NIO ET9 Launch Test",
    "year": "2024",
    "category": "Commercial",
    "one_line_zh": "蔚来 ET9 新品发布会的现场展播内容。负责音响系统展播片的剪辑：围绕核心卖点提炼内容、编排节奏，并按舞台大屏规格与展播时长完成适配——发布会级别的交付标准与排期要求。（页面展示为剪辑版本，不含终混音效、调色与特效）\n\n同期担任蔚来创始人访谈微综艺《斌哥和他的朋友们・音效工程师篇》的剪辑，梳理叙事主线，平衡人物表达与品牌调性。（该集因内部调整未播出）",
    "one_line_en": "Edited the NIO ET9 launch audio showcase reel — cut for stage screen, key selling points focused. Also edited an episode of NIO founder's interview series (unreleased).",
    "platform": "xinpianchang",
    "embed": "https://player.xinpianchang.com/?aid=13676130&mid=n2k9QAVkpzx718MZ",
    "link": "https://www.youtube.com/watch?v=_xHFJjDLPCY",
    "cover": "assets/img/che1.png",
    "cover_webp": "assets/img/che1.webp",
    "credits": [
      "剪辑 / Editor"
    ],
    "process": [
      "素材精剪 / Highlight Edit",
      "节奏与信息结构 / Rhythm & Structure"
    ],
    "tools": [
      "Premiere Pro"
    ],
    "stills": ["assets/img/nio-1.png"]
  }
];

window.WORK_CATEGORIES = [
  { key: "All", label: "全部 / All" },
  { key: "Documentary", label: "纪录/访谈 / Documentary" },
  { key: "Commercial", label: "品牌内容 / Brand" },
  { key: "ShortVideo", label: "短视频 / Short Video" },
  { key: "Short", label: "短片 / Videos" }
];

window.WORK_YEARS = [
  { key: "2026", label: "2026 / 2026" },
  { key: "2025", label: "2025 / 2025" },
  { key: "2024", label: "2024 / 2024" },
  { key: "2023", label: "2023 / 2023" },
  { key: "2022", label: "2022 / 2022" },
  { key: "2021", label: "2021 / 2021" }
];
